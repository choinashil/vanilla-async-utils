...
export default function groupBy (collection, iteratee, callback) {
    let count = 0;
    let done = false;
    let list = {};
    ...
    if (collection.length) {
        for (let i = 0; i < collection.length; i++) {
            iteratee(collection[i], (err, result) => cb(err, result, collection[i]));
        }
    } else {
        callback(null, {});
    }

    function cb(err, result, element) {
        count++;
        ...
        if (err && !done) {
            ...
            list[result] ? list[result].push(element) : list[result] = [element];
